...
pub fn run<C: Component>(root: &mut C) {
    ...
    root.place(Rect::new(0.0, 0.0, 800.0, 600.0));
        ...
        glfw.poll_events_unbuffered(|_, (_, event)| {
            ...
            match event {
                WindowEvent::Size(width, height) => {
                    root.place(Rect::new(0.0, 0.0, width as f32, height as f32));
                }
                    ...
                    if let Some(button) = match button {
                        ...
                        match action {
                            glfw::Action::Press | glfw::Action::Repeat => {
                                root.event(Event::MouseDown(button), &mut context);
                            }
                            glfw::Action::Release => {
                                root.event(Event::MouseUp(button), &mut context);
                            }
                        }
                    }
                }
                WindowEvent::CursorPos(x, y) => {
                    ...
                    root.event(Event::MouseMove, &mut context);
                }
                WindowEvent::Scroll(dx, dy) => {
                    root.event(Event::Scroll(dx as f32, dy as f32), &mut context);
                }
                WindowEvent::Key(key, _scancode, action, modifiers) => {
                    if let Some(key) = glfw_key(key) {
                        match action {
                            glfw::Action::Press | glfw::Action::Repeat => {
                                root.event(Event::KeyDown(key), &mut context);
                            }
                            glfw::Action::Release => {
                                root.event(Event::KeyUp(key), &mut context);
                            }
                        }
                    }
                }
                WindowEvent::Char(c) => {
                    root.event(Event::Char(c), &mut context);
                }
